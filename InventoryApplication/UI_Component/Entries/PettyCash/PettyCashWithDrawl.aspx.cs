...
{
    public partial class PettyCashWithDrawl : System.Web.UI.Page
    {
        ...
        private DataTable GetData()
        {
            ...
            try
            {
                dt = DAL.Helper.ListToDatatable.ToDataTable(DAL.Operations.OpPettyCash_WithDrawil.GetAll().Where(x => x.DomainCompanyId == DomainCompanyId).ToList());
            }
            catch (Exception ex)
            {
                lbl_message.Text = ex.Message;
        ...
        private void Populate_Control()
        {
            try
            {
                PettyCashTotal.InnerText = DAL.Operations.OpPettyCash_Total.GetRecordbyId(1).Amount;
            }
            catch (Exception ex)
            {
        ...
        protected void GV_One_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                ...
                DAL.Tbl_PettyCash_Withdrawl obj = new DAL.Tbl_PettyCash_Withdrawl
                {
                    Amount = Amount,
                ...
                int result = DAL.Operations.OpPettyCash_WithDrawil.InsertRecord(obj);
                if (result > 0)
                {
        ...
        protected void GV_One_RowUpdating(object sender, GridViewUpdateEventArgs e)
        {
            try
            {
                ...
                int result = DAL.Operations.OpPettyCash_WithDrawil.UpdateRecord(obj, id);
                if (result > 0)
                {
                    InsertPettyCash(PrevAmount, true);
                    InsertPettyCash(Amount, false);
        ...
        protected void GV_One_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            ...
            {
                int id = Convert.ToInt32(GV_One.DataKeys[e.RowIndex].Value.ToString());
                string Amount = DAL.Operations.OpPettyCash_WithDrawil.GetRecordbyId(id).Amount;

                if (DAL.Operations.OpPettyCash_WithDrawil.DeleteById(id))
                {
        ...
        private void InsertPettyCash(string Amount, bool toAdd)
        {
            try
            {
                ...
                if (Amount.Length > 0)
                {
                    ...
                    {
                        if (decimal.Parse(Amount) > decimal.Parse(amount))
                        {
                    ...
                    if (OKAY)
                    {
                        ...
                        DAL.Operations.OpLogger.Info("Petty Cash has been updated");
                    }
                    else
                    {
                        lbl_message.Text = "Amount Exceeded";
                    }
                }
