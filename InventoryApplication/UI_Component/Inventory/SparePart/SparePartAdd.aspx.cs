...
{
    public partial class PurchaseSparePartAdd : System.Web.UI.Page
    {
        ...
        protected void txt_Submit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    int recordID = CreatePurchaseSpareParts();

                    if (recordID > 0)
                    {
                        CreateAttachment(recordID);
                        lbl_message.Text = "Record added successfully";
                        DAL.Operations.OpLogger.Info("Inventory SpareParts record added by " + Session["Name"].ToString());
        ...
        private int CreatePurchaseSpareParts()
        {
            int result = -1;
            try
            {
                ...
                result = DAL.Operations.OpPurchase_SparePart.InsertRecord(obj);
            }
            catch (Exception ex)
            {
                lbl_message.Text = ex.Message;
